-- setup20260310.sql 기준 스키마 승격 마이그레이션
-- 목적: legacy 객체 정리 후 단일 스키마(cbt core)로 정렬

-- legacy 테이블/함수 정리(존재 시)
drop table if exists public.choices cascade;
drop table if exists public.submission_reports cascade;
drop table if exists public.submissions cascade;
drop table if exists public.reports cascade;

drop function if exists public.finalize_attempt(uuid, text, date, jsonb);
drop function if exists public.finalize_attempt(uuid, text, text, jsonb);

drop function if exists public.submit_exam(uuid, text, date, jsonb);
drop function if exists public.submit_exam(uuid, text, text, jsonb);
