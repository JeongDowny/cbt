...
export async function createClassYearAction(formData: FormData) {
  const yearValue = Number(asTrimmedString(formData.get("year")));
  if (!Number.isInteger(yearValue) || yearValue < 2000) {
    redirectDashboard("올바른 연도를 입력해 주세요.", "error");
  }

  const supabase = createSupabaseAdminClient();
  const { error } = await supabase.from("class_years").insert({ year: yearValue });

  if (error) {
    redirectDashboard("연도 저장에 실패했습니다.", "error");
  }

  revalidatePath(routes.adminDashboard);
  redirectDashboard("연도를 추가했습니다.");
}

export async function deleteClassYearAction(formData: FormData) {
  const id = asTrimmedString(formData.get("id"));
  if (!id) {
    redirectDashboard("삭제할 연도를 찾을 수 없습니다.", "error");
  }
  ...
  if (error) {
    redirectDashboard("사용 중인 연도는 삭제할 수 없습니다.", "error");
  }

  revalidatePath(routes.adminDashboard);
  redirectDashboard("연도를 삭제했습니다.");
}

export async function createClassNameAction(formData: FormData) {
  const name = asTrimmedString(formData.get("name"));
  if (!name) {
    redirectDashboard("반 이름을 입력해 주세요.", "error");
  }

  const supabase = createSupabaseAdminClient();
  const { error } = await supabase.from("class_names").insert({ name });

  if (error) {
    redirectDashboard("반 이름 저장에 실패했습니다.", "error");
  }

  revalidatePath(routes.adminDashboard);
  redirectDashboard("반 이름을 추가했습니다.");
}

export async function deleteClassNameAction(formData: FormData) {
  const id = asTrimmedString(formData.get("id"));
  if (!id) {
    redirectDashboard("삭제할 반 이름을 찾을 수 없습니다.", "error");
  }

  const supabase = createSupabaseAdminClient();
  const { error } = await supabase.from("class_names").delete().eq("id", id);

  if (error) {
    redirectDashboard("사용 중인 반 이름은 삭제할 수 없습니다.", "error");
  }

  revalidatePath(routes.adminDashboard);
  redirectDashboard("반 이름을 삭제했습니다.");
}

export async function createClassCohortAction(formData: FormData) {
  const cohortNo = Number(asTrimmedString(formData.get("cohortNo")));
  if (!Number.isInteger(cohortNo) || cohortNo < 1) {
    redirectDashboard("올바른 기수를 입력해 주세요.", "error");
  }

  const supabase = createSupabaseAdminClient();
  const { error } = await supabase.from("class_cohorts").insert({ cohort_no: cohortNo });

  if (error) {
    redirectDashboard("기수 저장에 실패했습니다.", "error");
  }

  revalidatePath(routes.adminDashboard);
  redirectDashboard("기수를 추가했습니다.");
}

export async function deleteClassCohortAction(formData: FormData) {
  const id = asTrimmedString(formData.get("id"));
  if (!id) {
    redirectDashboard("삭제할 기수를 찾을 수 없습니다.", "error");
  }

  const supabase = createSupabaseAdminClient();
  const { error } = await supabase.from("class_cohorts").delete().eq("id", id);

  if (error) {
    redirectDashboard("사용 중인 기수는 삭제할 수 없습니다.", "error");
  }

  revalidatePath(routes.adminDashboard);
  redirectDashboard("기수를 삭제했습니다.");
}
